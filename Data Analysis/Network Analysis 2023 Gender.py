...
x = messages[messages['chat_id']=='19:09c7dc9e-5871-4ba6-b595-4020749b1925_c8a1dcfa-bb67-401b-bba9-98023c68f78c@unq.gbl.spaces']
...
for chat_id in list(set(count_per_month_2023['chat_id'])):
    try:
        ...
        if count > mean_count:
            G1_gender.add_edge(party_1_id, party_2_id)
...
G1_gender = nx.relabel_nodes(G1_gender, lambda x: str(x))

## Too many nonconnected nodes 
non_connected_nodes = [node for node, degree in G1_gender.degree() if degree == 0]
#drop non connected nodes 
G1_gender.remove_nodes_from(non_connected_nodes)
...
net.from_nx(G1_gender)
net.show_buttons(filter_=["physics"])

# Customize nodes based on their degrees
degree_dict = dict(G1_gender.degree())
...
net.show("network_2023_sep_forward_genre.html")
...
eigenvector_centrality_2023 = nx.eigenvector_centrality(G1_gender)


# 5. Pagerank: Measures the importance of nodes using the PageRank algorithm.

pagerank_2023 = nx.pagerank(G1_gender)
...
edge_betweenness_2023 = nx.edge_betweenness_centrality(G1_gender)
